...
class SMTPConnection extends EventEmitter {
    ...
    _onClose() {
        ...
        } else if (!includes([this._actionGreeting, this.close], this._responseActions[0]) && !this._destroyed) {
    ...
    _setEnvelope(envelope, callback) {
        ...
        if (useSmtpUtf8 && includes(this._supportedExtensions, 'SMTPUTF8')) {
            args.push('SMTPUTF8');
        ...
        if (this._envelope.use8BitMime && includes(this._supportedExtensions, '8BITMIME')) {
            args.push('BODY=8BITMIME');
            this._using8BitMime = true;
        }

        if (this._envelope.size && includes(this._supportedExtensions, 'SIZE')) {
            args.push('SIZE=' + this._envelope.size);
        }

        // If the server supports DSN and the envelope includes an DSN prop
        // then append DSN params to the MAIL FROM command
        if (this._envelope.dsn && includes(this._supportedExtensions, 'DSN')) {
    ...
    _setDsnEnvelope(params) {
        ...
        if (notify) {
            ...
            let invaliNotify = notify.filter(n => !includes(validNotify, n));
            if (invaliNotify.length || (notify.length > 1 && includes(notify, 'NEVER'))) {
    ...
    _actionMAIL(str, callback) {
        ...
        if (!this._envelope.rcptQueue.length) {
            ...
            if (includes(this._supportedExtensions, 'PIPELINING')) {
